...
import { getCourseUsersCount } from "@/apiService/analytics";
import CountUp from "react-countup";
import { Loader2 } from "lucide-react";

export default function TeacherCount() {
  const { data, isLoading } = useQuery({
    queryKey: ["course-users-count"],
    queryFn: () => getCourseUsersCount(),
  });

  if (isLoading) return <Loader2 className='size-10 mx-auto animate-spin text-primary' />;
        ...
        <div className='relative flex flex-col items-center'>
          <div className='flex items-baseline gap-2'>
            <p className='text-5xl font-bold bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent'>
              <CountUp end={data?.teacher_count || 0} />
            </p>
          </div>

          <div className='px-4 mt-0.5 py-1.5 bg-primary/5 rounded-full'>
            <p className='text-sm font-medium text-primary/80'>Total Teachers</p>
          </div>
        </div>
      </div>
    </div>
