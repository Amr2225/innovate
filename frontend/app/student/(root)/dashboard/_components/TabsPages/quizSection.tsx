...
export default function QuizSection() {
  ...
  const { data: quizzes, isLoading } = useQuery({
    ...
    queryFn: () => getAssessment({ pageParam: 1, page_size: 10000, type: "Quiz", ...filter }),
  });
  ...
  return (
    <div>
      {dashboardLoading ? (
        ...
        <h1 className='text-xl font-bold mt-3'>Quizzes ({dashboardData?.quiz_count})</h1>
      )}
        ...
        {isLoading ? (
          ...
          quizzes?.data.map((quiz) => (
            <AssignmentCard
              ...
              hasSubmitted={quiz.has_submitted}
