...
class CourseCountView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        if user.role == "Student":
            ...
            submission_count = AssessmentSubmission.objects.filter(enrollment_id__in=enrollment_ids).count()
            submitted_assignments = AssessmentSubmission.objects.filter(enrollment_id__in=enrollment_ids, assessment__type='Assignment').count()
            submitted_exams = AssessmentSubmission.objects.filter(enrollment_id__in=enrollment_ids, assessment__type='Exam').count()
            submitted_quizzes = AssessmentSubmission.objects.filter(enrollment_id__in=enrollment_ids, assessment__type='Quiz').count()
            return Response({
            ...
            TeacherCount = User.objects.filter(role='Teacher', institution=user).count()
            StudentCount = User.objects.filter(role='Student', institution=user).count()
