...
class McqQuestionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        try:
            ...
            if 'question_grade' not in data:
                raise serializers.ValidationError("Question grade is required")

            logger.info("Validation completed successfully")
            return data
        except Exception as e:
            logger.error(
                f"Validation error in McqQuestionSerializer: {str(e)}")
        ...
        try:
            ...
            try:
                instance = self.Meta.model.objects.create(
                    ...
                    question_grade=validated_data.get(
                        'question_grade', Decimal('0.00')),
                    section_number=validated_data.get('section_number', 1),
                    created_by=request_user
                )
                logger.info(
                    f"Successfully created MCQ question with ID: {instance.id}")
                return instance
            except Exception as create_error:
                logger.error(
                    f"Error during model creation: {str(create_error)}")
    ...
    def to_representation(self, instance):
        try:
            ...
            raise serializers.ValidationError(
                f"Error processing question data: {str(e)}")
